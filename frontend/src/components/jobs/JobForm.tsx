...
import { CreateJobInput, JobStatus, LocationType } from "@/types/job";
...
export function JobForm({ editingId }: { editingId: string | null }) {
    ...
    useEffect(() => {
        if (existingJob) {
            setForm({
                ...
                applicationDate: dayjs(existingJob.applicationDate).format(
                    "YYYY-MM-DD",
                ),
                status: existingJob.status,
                noticePeriod: existingJob.noticePeriod || "",
                jobLink: existingJob.jobLink || "",
                tags: existingJob.tags,
                followUpSent: existingJob.followUpSent,
                followUpDate: existingJob.followUpDate
                    ? dayjs(existingJob.followUpDate).format("YYYY-MM-DD")
                    : undefined,
                timeoutDays: existingJob.timeoutDays,
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingId) {
            updateJob(
                { id: editingId, ...form },
                { onSuccess: () => dispatch(closeJobModal()) },
            );
        } else {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4">
                ...
                <Input
                    label="Applied on"
                    type="date"
                    value={form.applicationDate as string}
